...
const PasswordReset = () => {
    ...
    useEffect(() => {
        const verifyUrl = async () => {
          try {
            ...
            const backendUrl = `${import.meta.env.VITE_BACKEND_URL}/api/doctor/password-reset-var-c/${id}/${token}`;
            console.log("Verifying URL:", backendUrl); // log aaaaaaaaaaa
            await axios.get(backendUrl); // check with backend for URL verification aaaaaaaaaaaaa
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/doctor/password-reset-set-c/${id}/${token}`, { password });
            setMsg(data.message);
            setError("");
            navigate('/login')
        } catch (error) {
